...
export function CreateCategorySheet({ open, type, categories, onClose, onCreated }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 animate-fade-in">
      <button type="button" className="absolute inset-0" aria-label="Закрыть" onClick={onClose} />
      <form
        onSubmit={(e) => void submit(e)}
        className="relative z-10 flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-t-2xl bg-white shadow-xl animate-slide-up"
      >
        ...
        <div className="shrink-0 border-t border-neutral-100 bg-white px-4 pb-[calc(4.5rem+max(1rem,env(safe-area-inset-bottom)))] pt-3 lg:pb-[max(1rem,env(safe-area-inset-bottom))]">
